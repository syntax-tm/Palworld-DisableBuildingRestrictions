...
public abstract class MultiAddressModuleBase : ModuleBase
{
    ...
    public override async Task<bool> TryInitialize(Mem gameMemory)
    {
        ...
        var addresses = await GetDefaultAddresses().ConfigureAwait(false);
        ...
        if (addresses is null or { Count: 0 }) return false;
    ...
    private async Task<IReadOnlyList<string>> GetDefaultAddresses()
    {
        ...
        {
            var address = await GetAddress(_defaultAobs[i]).ConfigureAwait(false);
    ...
    private async Task<IReadOnlyList<string>> GetAlreadyEnabledAddresses()
    {
        ...
        {
            var address = await GetAddress(_alreadyEnabledAobs[i]).ConfigureAwait(false);
    ...
    {
        var foundAddresses = await GameMemory
            .AoBScan(_minAddress, _maxAddress, aobs, false, true)
            .ConfigureAwait(false);

        var address = foundAddresses?.FirstOrDefault();

        if (address is null or 0) return null;
